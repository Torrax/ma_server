...
CONF_MASS_PLAYER_ID = "mass_player_id"          # target MA player
...
class AudioInputProvider(PluginProvider):
    ...
    def __init__(
        ...
        # Resolve config
        self.mass_player_id: str = cast(str, self.config.get_value(CONF_MASS_PLAYER_ID))
        ...
        self._source_details = PluginSource(
            id=self.instance_id,
            name=self.name,
            passive=True,                       # cannot be chosen explicitly by users
    ...
    async def handle_async_init(self) -> None:
        ...
        if not self.player:
            self.logger.warning(
                "Target player %s not (yet) available – capture will start once it appears.",
                self.mass_player_id,
            )
        # monitor player add/remove so we can restart the pipe when needed
    ...
    def _on_mass_player_event(self, event: "MassEvent") -> None:
        """React to linked player lifecycle."""
        if event.object_id != self.mass_player_id:
            return

        if event.event == EventType.PLAYER_REMOVED:
            self.logger.info("Linked player removed – pausing capture.")
            self._stop_capture_daemon()
        elif event.event == EventType.PLAYER_ADDED:
            self.logger.info("Linked player is back – resuming capture.")
            self._start_capture_daemon()
    ...
    async def _capture_runner(self) -> None:
        ...
        while not self._stop_called:
            self._capture_proc = proc = AsyncProcess(
                ffmpeg_cmd, stdout=False, stderr=True, name=f"audio-capture[{self.name}]"
            )
            await proc.start()
